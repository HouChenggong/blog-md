...
现在我们来推导一下我们暴力的过程，我们定义sum[i]表示从开始加到第i位置的和，在数组[1,2,3,4]上我们就有
...
这说明sum[i] - sum[j] 就是(j,i)之间连续数组的和，如果sum[i] - sum[j] = k,那我们就可以进行计数了，这里也可以进行一次变形sum[j] = sum[i] - k，我们使用哈希表来记录sum[j],再次出现的时候我们就进行计数。
