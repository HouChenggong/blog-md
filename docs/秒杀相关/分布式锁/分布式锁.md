...
1. TTL时间>业务执行时间+redis加锁时间+时钟漂移时间

2. 加锁时间要很快，加锁成功标志：过半数
3. redisMaster挂了，slave一定要延迟TTL时间启动
4. redLock本质还是redis的分布式锁特性

### redLock

redis本身存在的问题：TTL 、时钟飘逸



Redis master挂了，一定要延迟TTL的时间启动
