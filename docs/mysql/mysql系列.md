...
- MVCC和CAS的区别
  ...
  - CAS修改有ABA问题
  - Mvcc修改自己在没有提交之前，锁住当前记录，别人无法修改

- MVCC作用

  - 读不加锁，普通的SELECT请求不会加锁，提高了数据库的并发处理能力
  - 保证了ACID中的I特性（隔离性）实现RC和RR级别
...
- 快照读
  ...
  - 读不加锁，读取记录的快照版本，而非最新版本，使用MVCC机制，最大的好处是读取不需要加锁，读写不冲突

- 快照生成时机

  - 开启事物执行了CURD操作必然会生成事物ID，但是不一定马上生成快照ReadView
  - 当我们第一次执行查询的时候，才会生成一个针对整个数据库ReadView
  - 如果不是查询语句，则不会生成快照ReadView，比如update、delete、insert语句都不会生成一个快照
  - 生成一次快照后，如果是RR（可重复读），则除了自己修改的数据外，其它数据保持不变，因为要维持可重复读；如果是RC（不可重复读），则在每一次select的时候会重新生成一个快照
  - RR第一次生成快照的时候，并不是查询比当前事物ID小的数据，而是查询已经commit之后的最新数据，可能会包含一个事物开启的比自己还晚的数据

####  undo log版本链

- 原理：借助两个隐藏字段data_trx_id、data_roll_ptr和undoLog日志实现版本链
  - trx_id：最近更新或创建 这条记录的 事务id
  - roll_pointer：回滚指针，指向了上一个修改的undoLog地址
...
|                | 说明                                     | 其它 |
| -------------- | ---------------------------------------- | ---- |
| m_ids          | 创建当前事务时，未提交的事物ID列表       |      |
| min_trx_id     | m_ids里最小的值                          |      |
| max_trx_id     | mysql下一个要生成的事务id,就是最大事务id |      |
| creator_trx_id | 当前事务ID                               |      |

那一个事务在执行快照生成的时候，如果鉴别我要取undo log版本链中的哪个记录呢？

| 情况                                          | 说明                  |
| --------------------------------------------- | --------------------- |
| 当前事务ID>max_trx_id                         | 不存在                |
| 当前事务ID<min_trx_id                         | 可读                  |
| min_trx_id<当前事务ID<max_trx_id且不在m_ids中 | 能访问m_ids中的版本   |
| min_trx_id<当前事务ID<max_trx_id在m_ids中     | 不能访问m_ids中的版本 |
|                                               |                       |
|                                               |                       |

比如先后开启ABCD4个事物，他们的事物时间线是这样的。在RR级别下

原本记录是0

| 事物A    | 事物B      | 事物C           | 事物D    | 时刻 |
| -------- | ---------- | --------------- | -------- | ---- |
| Update=1 | 开启事务   | 开启事务        | 开启事务 | T0   |
| update=2 | 做其它事情 | 做其它事情      | Read_A   | T1   |
| Commit   |            | Update-commit=3 |          | T2   |
|          | Update=4   |                 | Read_B   | T3   |
|          | Commit     |                 |          | T4   |
|          |            | Update=5        | Read_C   | T5   |
|          |            | Commit          |          | T6   |
...
- Read_A时刻读取的活跃事务ID列表为ABC，版本链为1——2
  - 那么Read_A虽然读到了版本2和1，但是它们都属于活跃事务列表中的，所以不算
  - 最终读取的结果是：0
- Read_B分在RR级别下，读的还是Read_A，也就是没有生成快照
  - 读到版本链1——2——3——4，先读到4和3在活跃事务ID列表中，不取
  - 再读到2和1在活跃事务ID列表中，不取，最终结果还是：0
- 同理RR级别下，Read_C和Read_D最终读取的结果都是：0
...
- Read_A时刻读取的活跃事务ID列表为ABC，版本链为1——2
  - 那么Read_A虽然读到了版本2和1，但是它们都属于活跃事务列表中的，所以不算
  - 最终读取的结果是：0
- Read_B分在RC级别下，重新生成快照，活跃事务列表为：B
  - 读到版本链1——2——3——4，先读到4在活跃事务ID列表中，不取
  - 再读到3，发现不在获取列表中且小于当前事务ID，取，最终结果是：3
- 同理RC级别下，Read_C读到的是4，Read_D读到的是5
...
- 版本链路是：0——1（他人提交）——2（自己修改）——自己回归
- 它会从最新的版本链路向前寻找到自己修改的数据，找到自己修改数据的回滚指针，如果中间已经有被提交了的，则终止寻找，回滚

为啥回滚的时候，不会回滚成原来自己修改前的版本呢？

​	比如A进来查询的时候，数据上V0，别人修改了变成了V1，但是其实他是看不到V1的，但是为啥回滚的时候居然回滚成了V1呢？可以简单的理解为：你只能回滚自己的操作，但是别人已经提交了的数据你是不能回滚的，不然别人就白提交了



因为undo log机制，undo log其实就是在你进行任何增删改（不包括查询）之前，记录数据库的最新记录undo log，然后回滚的时候直接用。即增删改执行的是当前读而非快照读
